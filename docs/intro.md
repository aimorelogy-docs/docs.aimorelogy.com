---
id: intro
slug: /
title: AIMORELOGY Documentation
description: Product manuals, integration guides, and technical resources from AIMORELOGY.
hide_table_of_contents: true
sidebar_position: 1
---

<div className="docs-home">
  <div className="docs-home__eyebrow">AIMORELOGY / TECHNICAL DOCUMENTATION</div>
  <p className="docs-home__lead">
    Find product manuals, integration guidance, and engineering resources for
    AIMORELOGY edge AI, UAV, and embedded products.
  </p>
  <div className="docs-home__actions">
    <a className="button button--primary button--lg" href="/products/ovis">Explore Ovis</a>
    <a className="button button--outline button--secondary button--lg" href="/products/ovis/getting-started">Get started</a>
  </div>
</div>

## Start here

<div className="docs-grid">
  <a className="docs-grid__item" href="/products/ovis">
    <span className="docs-grid__index">01 / PRODUCTS / OVIS</span>
    <strong>Ovis</strong>
    <span>Product overview, setup instructions, development resources, and downloads for Ovis.</span>
  </a>
  <a className="docs-grid__item" href="/products/ovis/secondary-development">
    <span className="docs-grid__index">02 / DEVELOPMENT</span>
    <strong>Secondary development</strong>
    <span>Integration guidance and development resources for building with Ovis.</span>
  </a>
</div>

:::info Documentation status

This documentation portal is being prepared for the first product publications. Product-specific manuals and downloads will appear here as they are released.

:::

## Support

If you cannot find what you need, visit the [AIMORELOGY website](https://aimorelogy.com) or join the [community forum](https://forum.aimorelogy.com).
